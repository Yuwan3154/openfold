...
The per-step train-loss metric is a data-order artifact; the smoke's EMA val barely moves in 100 steps;
the val data pipeline stalls on post-cutoff proteins. So: from the TRAIN pipeline (fast) cache K eval
batches (held out from the training steps), then eval the RAW weights on that FIXED cache before/after
N steps of conf+structure training at conf-weight M:
  before = eval(init) on K cached batches      [struct=fape+disto+mmsa+chi+viol; conf=plddt+exp_res+tm @ AF2 M=1]
  train N steps at conf-weight M (evoformer-only, frozen conf heads, Adam lr, grad-clip 0.1, bf16 autocast)
  after  = eval(raw weights) on the SAME K cached batches
struct_delta>0 = structure DEGRADED; conf_delta<0 = confidence loss IMPROVED. One M per GPU (4-way).
"""
import argparse
import copy
...
L_train = mk_loss({**w_struct, **{k: CONF_W[k] * args.mult for k in CONF}})
...
dm = OpenFoldDataModule(
    ...
    batch_seed=42, train_epoch_len=args.k_eval + args.steps + 5)
dm.prepare_data()
dm.setup()
it = iter(dm.train_dataloader())

# Cache K eval batches (held out from the N training steps that follow) -> fixed before/after set.
eval_batches = [prep(next(it)) for _ in range(args.k_eval)]
print(f"M={args.mult} cached {len(eval_batches)} eval batches", flush=True)
...
for step in range(args.steps):
    ...
    opt.zero_grad()
    loss.backward()
    torch.nn.utils.clip_grad_norm_(m.evoformer.parameters(), 0.1)
    opt.step()
    if step % 20 == 0:
        print(f"M={args.mult} step {step} train_loss={float(loss):.3f}", flush=True)
    del out, loss

s_after, c_after = evaluate()
